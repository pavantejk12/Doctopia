...
async function page() {
  ...
  return (
    <div>
      <h1 className='text-black text-xl font-semibold'>Flashcards</h1>
      <div className='grid lg:grid-cols-2 gap-3 grid-col-1 mt-3'>
        {data.docs.map((subject:any,index:number)=>{
          return (
            <Link className={`${subject.isqbank === 'True' ?'block':'hidden'}`}  key={index} href={`/cards/${subject?.id??''}`}>
      ...
      <div className='grid lg:grid-cols-2 gap-3 grid-col-1 mt-3'>
        {data.docs.map((subject:any,index:number)=>{
          return (
            <Link className={`${subject.isqbank === 'True' ?'hidden':'block'}`} key={index} href={`/cards/${subject?.id??''}`}>
